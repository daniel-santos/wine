 ...
 *   ENOSYS
 */
static void DECLSPEC_NORETURN DECLSPEC_NOINLINE DECLSPEC_COLD
fatal_futex_error(const char *op, int *addr)
{
    int err = errno;
    perror(op);
 ...
 * hash to match it */
static DECLSPEC_COLD enum shm_sync_value_result sync_fail( struct hybrid_sync_object *ho,
                                                           const char *fmt, ... )
{
    if (fmt)
    {
...
static DECLSPEC_COLD void bad_hash( struct hybrid_sync_object *ho, union shm_sync_value *pre_ptr,
                                    int hash )
{
    unsigned a = hash;
...
check_data( struct hybrid_sync_object *ho, union shm_sync_value *pre_ptr, int maybe_stale,
            int wait_lock )
{
    int flags;
    int flags_hash;
             ...
             union shm_sync_value *post_ptr, int data, int flags )
{
    ...
    if (!WINESERVER)
        ...
        assert( !(flags & (SHM_SYNC_VALUE_LOCKED | SHM_SYNC_VALUE_MOVED
 ...
 *
 */
NTSTATUS hybrid_semaphore_init( struct hybrid_semaphore *sem, unsigned int initial,
                                unsigned int max )
{
 ...
 *
 */
static DECLSPEC_NOINLINE DECLSPEC_COLD NTSTATUS
wine_sync_value_result_to_ntstatus( enum shm_sync_value_result value )
{
    switch ((int)value)
    {
 ...
 */
NTSTATUS __hybrid_semaphore_op( struct hybrid_semaphore *sem, union shm_sync_value *pre_ptr,
                                int maybe_stale, int change )
{
...
static void timespec_sub( struct timespec *dest, const struct timespec *a,
...
static inline void timespec_remain( struct timespec *dest, const struct timespec *start,
 ...
 */
NTSTATUS __hybrid_mutex_op( struct hybrid_mutex *mutex, union shm_sync_value *pre_ptr,
                            int maybe_stale, int change )
{
...
/* dump functions */
static void wine_sync_value_dump( const union shm_sync_value *value, char **start,
...
void hybrid_object_dump( const struct hybrid_sync_object *ho, char **start, const char *const end )
{
    ...
    {
        count = snprintf( *start, end - *start, "%p {value = %s", ho,
                          ho->atomic.value ? "" : "(NULL)" );
        if (count < 0)
            goto error;
...
void hybrid_semaphore_dump( const struct hybrid_semaphore *sem, char **start,
                            const char *const end )
{
    int count;
