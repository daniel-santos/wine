...
#define HYBRID_SYNC_FLAGS_BITS      4   /* keep an even number */
...
/* FIXME: We're no longer using atomic instructions on this whole union, only on flags_refcounts,
...
#if 0       /* Layout for reference only.  We can't use bitfields because we need to
             * know which 32-bit portion contains flags and refcount.  */
            struct {
                unsigned long flags:HYBRID_SYNC_FLAGS_BITS;
                unsigned long refcount:HYBRID_SYNC_REFCOUNT_BITS;
...
/* Always bits 0-3. */
static inline unsigned long hso_atomic_get_flags( const union hso_atomic *atomic )
{
...
/* 32-bit: bits 4-17
 * 64-bit: bits 4-33 */
static inline unsigned long hso_atomic_get_refcount( const union hso_atomic *atomic )
{
...
/* 32-bit: bits 18-31
 * 64-bit: bits 34-63 */
static inline unsigned long hso_atomic_get_waitcount( const union hso_atomic *atomic )
{
     return atomic->flags_refcounts >> (HYBRID_SYNC_FLAGS_BITS + HYBRID_SYNC_REFCOUNT_BITS);
}

static inline unsigned long hso_atomic_make_flags_refcounts( unsigned long flags,
...
/* Return a pointer to the 32-bit portion of flags_refcounts that contains flags
 * and lowest bits of refcount. */
...
/* Semaphore Functions */
extern NTSTATUS hybrid_semaphore_init( struct hybrid_semaphore *sem, unsigned int initial,
                                       unsigned int max) SYNC_H_ATTR;
extern NTSTATUS hybrid_semaphore_release( struct hybrid_semaphore *sem, unsigned int count,
                                          unsigned int *prev) SYNC_H_ATTR;
extern NTSTATUS __hybrid_semaphore_op( struct hybrid_semaphore *sem, union shm_sync_value *pre_ptr,
                                       int maybe_stale, int change) SYNC_H_ATTR;
extern NTSTATUS __hybrid_semaphore_trywait_trans_op( struct hybrid_semaphore *sem,
                                                     union shm_sync_value *trans_state,
                                                     enum hybrid_sync_trans_op op,
                                                     int clear_notify) SYNC_H_ATTR;
extern NTSTATUS hybrid_semaphore_wait( struct hybrid_semaphore *sem,
                                       const struct timespec *timeout ) SYNC_H_ATTR;

/* Non-transactional semaphore trywait. */
...
/* Attempt a transactional semaphore trywait.  If the trywait fails, a transaction will not be
 * started and the object will be marked with "server notify" instead. */
static inline NTSTATUS hybrid_semaphore_trywait_begin_trans( struct hybrid_semaphore *sem,
                                                             union shm_sync_value *trans_state)
{
    return __hybrid_semaphore_trywait_trans_op( sem, trans_state, HYBRID_SYNC_TRANS_OP_BEGIN,
                                                FALSE );
}

/* Commit a transactional semaphore trywait after a sucessful call to
 * hybrid_semaphore_trywait_begin_trans.  The object will be unlocked when this function
 * returns. */
static inline NTSTATUS hybrid_semaphore_trywait_commit( struct hybrid_semaphore *sem,
                                                        union shm_sync_value *trans_state,
                                                        int clear_notify)
{
    return __hybrid_semaphore_trywait_trans_op( sem, trans_state, HYBRID_SYNC_TRANS_OP_COMMIT,
                                                clear_notify );
}

/* Rollback a transactional semaphore trywait after a sucessful call to
 * hybrid_semaphore_trywait_begin_trans.  The object will be unlocked when this function
 * returns. */
static inline NTSTATUS hybrid_semaphore_trywait_rollback( struct hybrid_semaphore *sem,
                                                          union shm_sync_value *trans_state )
{
    return __hybrid_semaphore_trywait_trans_op( sem, trans_state, HYBRID_SYNC_TRANS_OP_ROLLBACK,
                                                FALSE );
}

/* Mutex Functions (not yet implemented) */
...
/* dump functions */
extern void hybrid_object_dump( const struct hybrid_sync_object *ho, char **start,
                                const char *const end) SYNC_H_ATTR;
extern void hybrid_semaphore_dump( const struct hybrid_semaphore *sem, char **start,
                                   const char *const end) SYNC_H_ATTR;
extern void hybrid_mutex_dump( const struct hybrid_mutex *mutex, char **start,
                               const char *const end) SYNC_H_ATTR;
extern void shm_object_info_dump( const struct shm_object_info *info, char **start,
                                  const char *const end) SYNC_H_ATTR;

static inline void shared_sync_assert_sizes( void )
{
