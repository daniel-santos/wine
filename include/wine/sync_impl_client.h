 ...
 *
 */
static DECLSPEC_COLD DECLSPEC_NOINLINE enum shm_sync_value_result
check_data_anomalous( struct hybrid_sync_object *ho, union shm_sync_value *pre,
                      enum shm_sync_value_result flags, int wait_lock )
{
    ...
    {
        if (wait_lock)
            return hybrid_object_wait_global_lock( ho, pre );
        else
	...
	{
	    case STATUS_SUCCESS:
                pre->int64 = locked_read64( &ho->atomic.value->int64 );
 ...
 */
static void DECLSPEC_NOINLINE hybrid_object_do_move_wait(struct hybrid_sync_object *ho,
                                                         union hso_atomic *pre)
{
