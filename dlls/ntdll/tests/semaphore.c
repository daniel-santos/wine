...
static NTSTATUS DECLSPEC_MUST_CHECK (*pntdll_object_release)(struct ntdll_object *obj);
...
static void test_limits(void)
{
    ...
    do_test_ae( s, !s, "%p", err == ERROR_INVALID_PARAMETER,
                CreateSemaphoreA, NULL, (ULONG)INT_MAX + 1u, (ULONG)INT_MAX + 1u, NULL );
    if (s)
        CloseHandle( s );

    do_test_ae( s, !s, "%p", err == ERROR_INVALID_PARAMETER,
                CreateSemaphoreA, NULL, INT_MAX, (ULONG)INT_MAX + 1u, NULL );
    if (s)
        CloseHandle( s );

    do_test_ae( s, !s, "%p", err == ERROR_INVALID_PARAMETER,
                CreateSemaphoreA, NULL, (ULONG)INT_MAX + 1u, INT_MAX + 1u, NULL );
    if (s)
        CloseHandle( s );
...
static void test_sem_simple_wait(void)
{
    HANDLE sem;
    DWORD ret;
    do_test_ae( sem, !!sem, "%p", !err,
                CreateSemaphoreA, NULL, 0, 1, NULL );
    do_test_ae( ret, ret == WAIT_TIMEOUT, "%d", err == 0xdeadbeef,
...
static int wait_for_value_and_set(volatile int *addr, int expected_value, int new_value, DWORD duration)
{
    ...
    int cur_value = InterlockedExchangeAdd(addr, 0);
    ...
    while ((LONGLONG)(end - GetTickCount64()) > 0ll)
    {
        ...
        usleep (4);
        //Sleep(4);
        cur_value = InterlockedExchangeAdd(addr, 0);
    }
...
if (0)
{
        ...
        if (objs[i])
        {
            NTSTATUS status = pntdll_object_release(objs[i]);
